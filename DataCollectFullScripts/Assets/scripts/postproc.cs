...
public class postproc : MonoBehaviour {
	...
	private int frames_to_skip = 120;
	...
	public void save_coco_json(int id, int width, int height, string image_name, string camera){
		...
		if(new_file_coco) 
		{
			File.WriteAllText($"Data/ImageJSON/{camera}_image.json", "[" + coco_json);
			new_file_coco = false;
		}else{
			File.AppendAllText($"Data/ImageJSON/{camera}_image.json", 
    ...
    //    Destroy(tex);
	//
	//	string depth_file_path = $"Data/DepthImage/depthTexture_{cam.name}_{image_numb}.png";
